...
strings = ["sea_water_temperature", "sea_water_temperature [celsius]", "water_temperature", "temperature_qc"]
...
def test_exclude():
    ...
    matches = ["sea_water_temperature", "sea_water_temperature [celsius]", "water_temperature"]
...
def test_exclude_start_include():
    ...
    reg = cfp.Reg(exclude_start=["water", "temperature"], include_or=["temp","celsius"])
    dfmatch = df[df.str.match(reg.pattern())]    
    matches = ["sea_water_temperature", "sea_water_temperature [celsius]"]
    tm.assert_series_equal(dfmatch, pd.Series(matches), check_index=False)
...
def test_exclude_start_end():
    reg = cfp.Reg(exclude_end=r"\[celsius\]", exclude_start="temperature", exclude="sea")
    dfmatch = df[df.str.match(reg.pattern())]    
    matches = ["water_temperature"]
    tm.assert_series_equal(dfmatch, pd.Series(matches), check_index=False)
...
def test_exclude_start_end_include_start_end():
    reg = cfp.Reg(exclude_start="temperature", exclude="celsius", include="water", include_start="sea")
    dfmatch = df[df.str.match(reg.pattern())]    
    matches = ["sea_water_temperature"]
    tm.assert_series_equal(dfmatch, pd.Series(matches), check_index=False)
