...
class Reg(object):
    ...
    * The `include` option is logical "and", `include_or` is logical "or", and the other `include_` options allow for only one selection. If you want to use more than one `include_start` for example, you should make an additional regular expression. 
    ...
    def __init__(self, 
        ...
        self._exclude = [] if exclude is None or exclude == "" else astype(exclude, list)
        self._exclude_start = [] if exclude_start is None or exclude_start == "" else astype(exclude_start, list)
        self._exclude_end = [] if exclude_end is None or exclude_start == "" else astype(exclude_end, list)
        self._include = [] if include is None or include == "" else astype(include, list)
        self._include_or = [] if include_or is None or include_or == "" else astype(include_or, list)
        self._include_exact = "" if include_exact is None or include_exact == "" else include_exact
        self._include_start = "" if include_start is None or include_start == "" else include_start
        self._include_end = "" if include_end is None or include_end == "" else include_end
    ...
    def check(self):
        ...
        others = [self._exclude, self._exclude_start, self._exclude_end, 
                  self._include, self._include_or, self._include_end, self._include_start]
        if (len(self._include_exact) > 0) and any([len(attr)>0 for attr in others]):
            raise ValueError("If `include_exact` is used, do not input any other options.")
